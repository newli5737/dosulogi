...
import { OpportunityModal } from '../../opportunity-modal/ui/OpportunityModal'
...
export function OpportunityTable() {
  ...
  const fetchPage = useCallback(
    (page, limit, filters) => opportunityApi.list(token, page, limit, filters),
    [token],
  )

  const { rows, meta, page, setPage, loading, reload } = usePaginated(fetchPage, { filters: { stage } })
  ...
  return (
    <>
      ...
      <OpportunityModal open={!!modal} edit={modal?.id ? modal : null} onClose={() => setModal(null)} onSaved={reload} />
    </>
  )
}
